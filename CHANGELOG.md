...
## v1.8.5 — 2026-08-05
El aprendizaje de v1.8.3/v1.8.4 aplicado al **funcionamiento** del plugin, no solo a su conocimiento:
- **Skill nueva `/medir-diversion`** (de 8 a 9 skills): el método que rescató el combate de Dragon Age: Inquisition, convertido en procedimiento — rondas cortas, obligatorias y recurrentes, con escenarios construidos a propósito y una encuesta numérica fija; lo que importa es la pendiente entre rondas, no el valor. Incluye adaptación para dev solo o dúo (3-5 testers externos fijos) y el gate honesto de cuándo el problema es el loop y no el pulido.
- **`templates/scope-peelable/SCOPE.md`** (plantilla nueva, ya la copia el scaffold): alcance por capas — qué sale sí o sí, qué se añade con un mes / seis / un año, y qué se corta si no hay ni un día. La técnica con la que BioWare negoció el retraso de Inquisition, para decidirlo en frío antes de que haya prisa.
- **`DevTools.cs` en el template día 0**: menú `El Estudio` en el editor con arranque desde la escena inicial, velocidad x0.25/x1/x4, borrado de progreso y acceso al CHECKPOINT. Aplica la lección más citada del libro — el bucle cambio→verlo es el factor número uno — al proyecto que genera el scaffold. Verificado compilando en Unity 6000.3.18f1: 0 errores, tests 1/1.
- **Agentes**: el `director` recibe cinco reglas duras (solo se enseña lo que va a shipear; estima solo lo probado; cuenta con el retraso; nombra la gravedad del equipo; separa lo que falló por tiempo de lo que falló por decisión); el `qa`, cómo priorizar cuando no da tiempo (el 60% antes que el 95%, el 90% no es terminado, un prototipo no es una feature); el `ingeniero`, arreglar el bucle de iteración antes de construir features.
- **Skills existentes**: `/juice-pass` incorpora el feel de UI y para en seco si el problema es el loop; `/nuevo-juego` rellena `SCOPE.md` en el día 0 y valida el prototipo con `/medir-diversion` en vez de con la opinión del autor; `/ship-check` añade la regla de marketing (solo material que está en el juego).
- **`docs/agent-routing-contract.md`**: definición explícita de "terminado" — el 90% no cuenta, prototipo ≠ feature, y la diversión se cierra con una serie que sube.
